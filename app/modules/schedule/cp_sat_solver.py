...
from app.config import SOLVER_TIMEOUT_SECONDS, MOLD_SETUP_MINUTES, WORK_HOURS_PER_DAY
...
def solve_with_cp_sat(
    ...
    for order in orders_to_schedule:
        for line in lines:
            ...
            interval = model.NewOptionalIntervalVar(
                start_var, work_minutes, end_var, presence,
    ...
    for key, info in order_line_map.items():
        ...
        if objective == ObjectiveType.MAX_ON_TIME:
            ...
            model.Add(end_var <= due_minutes).OnlyEnforceIf(presence)
            model.Add(end_var > due_minutes).OnlyEnforceIf(presence.Not())
            ...
            obj_exprs.append(weight * info["work_minutes"])

    if objective == ObjectiveType.MAX_UTILIZATION:
        for line_id, intervals in all_intervals_by_line.items():
            pass

    if objective == ObjectiveType.MIN_DELAY:
    ...
    if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        for key, info in order_line_map.items():
            if solver.BooleanValue(info["presence"]):
                ...
                setup_minutes = MOLD_SETUP_MINUTES
